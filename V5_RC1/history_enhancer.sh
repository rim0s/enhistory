#!/bin/bash
# SSH命令增强记录器 v1.5 - 支持数据库转录接口
# 记录格式：时间戳 会话类型 [PID] 用户名 执行的命令

# 配置变量（由部署脚本设置）
# ENHANCED_HISTORY_ENABLED=1
# ENHANCED_HISTORY_LOG_FILE="${HOME}/.enhanced_history"
...
get_session_info() {
    if [ -n "$SSH_CONNECTION" ]; then
        ...
        echo "[SSH] [su] [${preconn[0]}:${conn[1]}] TO [${conn[2]}:${conn[3]}] [$(tty | sed 's/\/dev\///')] [$(logname)] AS"
    else
...
enhistory() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            *)
                echo "未知参数: $1"
                echo "用法: enhistory [--all] [--since YYYY-MM-DD] [--from \"YYYY-MM-DD HH:MM\"] [--to \"YYYY-MM-DD HH:MM\"] [--db]"
                return 1
                ;;
        esac
    done
