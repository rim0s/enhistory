    ...
    if [ -z "$ENHANCED_PROCESS_CHAIN_CACHE" ] || [ "$ENHANCED_PROCESS_CHAIN_TS" != "$$" ]; then
        ...
        build_short_process_chain() {
            ...
            for ((i=0; i<max_depth; i++)); do
                local proc_info=$(ps -p $pid -o comm=,pid= --no-headers 2>/dev/null)
                if [ -n "$proc_info" ]; then
                    local proc_name=$(echo "$proc_info" | awk '{print $1}')
                    local proc_pid=$(echo "$proc_info" | awk '{print $2}')
                    # 简化进程名（去掉路径）
                    proc_name=$(basename "$proc_name")
                    processes=("${proc_name}(${proc_pid})" "${processes[@]}")
                    pid=$(ps -p $pid -o ppid= --no-headers 2>/dev/null | tr -d ' ')
                    [ "$pid" -le 1 ] && break
                else
                    break
                fi
        ...
        chain=$(build_short_process_chain)
...
# 增强的会话信息获取
get_session_info() {
    local tty_info=$(tty 2>/dev/null | sed 's/\/dev\///' || echo "notty")
    
    # 改进的SSH会话检测
    if [ -n "$SSH_CONNECTION" ] || is_ssh_session; then
        # SSH会话
        local source_ip=":"
        local source_port=":"
        local dest_ip=":"
        ...
        if [ -n "$SSH_CONNECTION" ]; then
            ...
            # 尝试从其他方式获取SSH信息
            source_ip=$(who am i | awk '{print $5}' | sed 's/[()]//g')
            [ -z "$source_ip" ] && source_ip=":"
        fi
        
        if [ "$(whoami)" != "${ENHANCED_SESSION_START_USER:-$(whoami)}" ]; then
            # SSH会话中的用户切换
            echo "[SSH] [su] [${source_ip}:${source_port}] TO [${dest_ip}:${dest_port}] [${tty_info}] [${ENHANCED_SESSION_START_USER:-$(whoami)}] AS"
        else
            # 普通SSH会话
            echo "[SSH] [${source_ip}:${source_port}] TO [${dest_ip}:${dest_port}] [${tty_info}]"
        fi
    else
        # 本地会话
        if [ "$(whoami)" != "${ENHANCED_SESSION_START_USER:-$(whoami)}" ]; then
            echo "[LOCAL] [su] [${tty_info}] [${ENHANCED_SESSION_START_USER:-$(whoami)}] AS"
        else
            echo "[LOCAL] [${tty_info}]"
        fi
    fi
